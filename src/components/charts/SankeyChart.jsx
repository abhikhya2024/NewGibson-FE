...
export default function AttorneyFirmSankeyChart() {
  ...
  useEffect(() => {
    const fetchAttByFirm = async () => {
      try {
        const res = await fetch(`${process.env.REACT_APP_PROD_API_URL}/api/attorney/`);
        const data = await res.json();

        if (Array.isArray(data.attorneys) && data.attorneys.length > 0) {
          // Map API response to {attorney, firm} pairs
          const pairs = data.attorneys.map(a => ({
            attorney: a.name,
            firm: a.law_firm || "Unknown Firm",
          }));
          setAttByFirm(pairs);
        } else {
